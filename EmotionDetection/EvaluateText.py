...
def evaluate(textFile, valueFile=None):
    ...
    if valueFile:
        ...
        viewPlot = askokcancel("Confusion matrix", "View confusion matrix plot?")
        if viewPlot is True:
            # normaliseData = raw_input("\nNormalise plot? [y/n]:").lower() in ['yes', 'y']
            normaliseData = askokcancel("Normalise plot", "Do you want to normalise plot?")
            if normaliseData is True:
